...
class DailyTask:
    ...
    focus_area: Optional[str] = None  # 当日重点（condition / assessment driven）
...
class PlanGenerator:
    ...
    def _load_activity_templates(self, language: str = 'en') -> Dict:
        ...
        templates = {
            ...
            'social': [
                {
                    ...
                    'duration': 10,
                    'description': 'Match faces to feelings — builds social attention and emotion recognition.',
                    'game_type': 'emotion_match',
                    'detailed_instructions': 'Play together. Name each feeling aloud. Praise effort, not speed.',
                    'can_play_online': True,
                    'min_age': 3,
                },
                {
                    ...
                    'duration': 10,
                    'description': 'Simple back-and-forth play to practice waiting and responding.',
                    'game_type': 'turn_taking',
                    'detailed_instructions': 'Take turns tapping. Model patience and celebrate when your child waits.',
                    'can_play_online': True,
                    'min_age': 3,
                },
                {
            ...
            'motor': [
                {
                    'type': 'online_game',
                    'name': 'Trace path (fine motor)',
                    'duration': 10,
                    'description': 'Tracing builds hand–eye coordination (OT-informed fine motor training).',
                    'game_type': 'trace_path',
                    'detailed_instructions': 'Trace slowly along the path. Praise effort and steady hands.',
                    'can_play_online': True,
                    'min_age': 3,
                },
                {
    ...
    def generate_plan(self, child_info: ChildInfo, test_results: List[TestResult], 
        ...
        try:
            ...
            # 分析测试结果 + 孩子状况，确定重点改善领域
            from models.condition_training import focus_areas_for_condition

            focus_areas = focus_areas_for_condition(
                child_info.child_condition,
                self._analyze_test_results(test_results, child_info),
            )
    ...
    def _generate_daily_tasks(self, child_info: ChildInfo, test_results: List[TestResult],
        ...
        from models.condition_training import test_type_for_child, game_type_for_activity

        for day in range(1, duration_days + 1):
            task_date = (start_date + timedelta(days=day - 1)).strftime('%Y-%m-%d')
            day_focus = focus_areas[(day - 1) % len(focus_areas)] if focus_areas else 'attention'

            # 生成每日活动（根据年龄过滤）
            activities = self._generate_day_activities(
                focus_areas, performance_level, day, duration_days, child_info.age
            )
            # Condition-specific online games on rotation
            for act in activities:
                if act.get('can_play_online') and act.get('type') == 'online_game':
                    gt = game_type_for_activity(child_info.child_condition, day_focus, day)
                    if gt:
                        act['game_type'] = gt
            ...
            test_type = None
            if test_required:
                age = int(child_info.age) if child_info.age else 6
                test_type = test_type_for_child(child_info.child_condition, age)
                logger.info(
                    "Day %s test: age=%s condition=%s test_type=%s",
                    day,
                    age,
                    child_info.child_condition,
                    test_type,
