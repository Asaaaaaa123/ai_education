"""
Training plan API — persists plans; child + test data live in child_profile_service (unified ChildProfile v2).

Post–Clerk onboarding uses /api/children first; these routes stay for plan tasks, progress, and backward compatibility.
"""
...
def _stored_profile_to_child_info(child_data: Dict) -> ChildInfo:
    """Strip unified ChildProfile fields so ChildInfo dataclass accepts storage dicts."""
    allowed = {f.name for f in fields(ChildInfo)}
    return ChildInfo(**{k: v for k, v in child_data.items() if k in allowed})
...
async def run_plan_generation(
    ...
    """Shared core for POST /api/plans/plans and POST /api/children/{id}/generate-plan."""
    ...
    child = _stored_profile_to_child_info(child_data)
    ...
    attach_plan(user.id, request.child_id, plan.plan_id)
...
async def create_plan(
    ...
    """创建训练计划（关联到当前用户）"""
    try:
        return await run_plan_generation(user, request, http_request)
    except HTTPException:
        raise
    except Exception as e:
