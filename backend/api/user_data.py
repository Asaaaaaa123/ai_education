...
from user_data import (
    ...
    get_recommendation_checklist, merge_recommendation_checklist,
)
...
class ChecklistPutRequest(BaseModel):
    scope_key: str
    items: Dict[str, bool]
...
async def get_recommendation_checklist_api(
    scope: str,
    http_request: Request,
    user: UserResponse = Depends(get_current_user),
):
    """Load saved checklist state for a scope (e.g. plan_goals_<planId>)."""
    try:
        data = get_recommendation_checklist(user.id, scope)
        return {"success": True, "data": {"scope": scope, "items": data}}
    except Exception as e:
        logger.error("get checklist failed: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail=str(e)) from e


@router.put("/checklist", response_model=Dict)
async def put_recommendation_checklist_api(
    body: ChecklistPutRequest,
    http_request: Request,
    user: UserResponse = Depends(get_current_user),
):
    """Persist checklist checkbox states into user_data.json."""
    try:
        merged = merge_recommendation_checklist(user.id, body.scope_key, body.items)
        return {"success": True, "data": {"scope": body.scope_key, "items": merged}}
    except Exception as e:
        logger.error("put checklist failed: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail=str(e)) from e
