...
# Data storage paths (same writable root as auth.py / plans)
...
def get_user_data(user_id: str) -> Dict:
    ...
    row = all_data[user_id]
    if 'recommendation_checklists' not in row:
        row['recommendation_checklists'] = {}
        all_data[user_id] = row
        save_user_data(all_data)
...
def get_recommendation_checklist(user_id: str, scope_key: str) -> Dict[str, bool]:
    """Return id->checked map for a checklist scope (e.g. plan goals, daily tips)."""
    ud = get_user_data(user_id)
    raw = (ud.get('recommendation_checklists') or {}).get(scope_key) or {}
    return {str(k): bool(v) for k, v in raw.items()}


def merge_recommendation_checklist(user_id: str, scope_key: str, items: Dict[str, bool]) -> Dict[str, bool]:
    """Merge checkbox states into user_data.json (MVP persistence)."""
    ud = get_user_data(user_id)
    checklists = ud.setdefault('recommendation_checklists', {})
    current = dict(checklists.get(scope_key) or {})
    for k, v in items.items():
        current[str(k)] = bool(v)
    checklists[scope_key] = current
    ud['recommendation_checklists'] = checklists
